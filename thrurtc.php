     ...
     $('.opns').on("click", function() {
         ...
         var formData = {
          ...
          $.ajax({
            ...
            //mins.append(dateTime);
            //$("#how_many").text("0");
              //$('#basicModal').modal('show');
              $('.toast').toast('show');
              //alert(data['message']);

              //var popup = $("#update_message");
              //document.getElementById("update_message").innerHTML = data['message'];
              //popup.append('<h5>'+data['message']+'</h5>'); 
            //alert(data['message']);
...
<div class="position-fixed top-1 end-0" style="z-index: 11" data-bs-autohide="false">
  ...
  aria-atomic="true">
    ...
    <?PHP 
/*
    $update = "UPDATE list SET rtc = '9' WHERE rtc='Regional Training Center - Eastern Samar'";
    $status_update = mysqli_query($db,$update);
    if($status_update){
      echo "<h2>RTC Changed to 9</h2>";
    }
    else {
        echo "ERROR: Could not able to execute $update. " . mysqli_error($db);  
      echo "<h2>Error while changing your password!</h2>";
    }
    die();
*/
    ?>
        ...
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <h5 class="card-title"><?=$_GET['r']?></h5>
              <p>Listed below are trainees in <?=$_GET['r']?> that are under monitoring and/or Academic Board.</p>
              ...
              <table class="table datatable">
                ...
                <tbody>
                  <?php 
                    $get_trainees = mysqli_query($db,"select * from list WHERE rtc=$_GET[rtc] AND nature_termi != 0");
                      ...
                      <tr>
                        ...
                        <td><a href="thrurtc.php?rtc=<?=$rows2['rtc']?>&r=<?=$x3?>"><?=$x3?></a></td>
                        <td>
                        ...
                        <td><button id="<?=$rows2['id']?>" value="<?=$rows2['id']?>" class="opns bi bi-pencil btn btn-info"></button></td>
                      </tr>
                  <?php
                        }
                      }
